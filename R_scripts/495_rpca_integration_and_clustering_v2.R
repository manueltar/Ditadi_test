...
rpca_and_cluster_at_low_res = function(option_list)
{
  ...
  adata<-readRDS(file=opt$filt_clustered_QCed_reclustered)
  ...
  combined.sct <- FindClusters(combined.sct, algorithm=4, resolution = opt$res_param, verbose=FALSE, method = "igraph")
  ...
  saveRDS(combined.sct, file="merged_unprocessed_db_filt_clustered_QCed_reclustered_rpca_integrate_clustered.rds")
  ...
  saveRDS(combined.sct.markers, file="merged_unprocessed_db_filt_clustered_QCed_reclustered_rpca_integrate_clustered_MARKER_GENES.rds")
...
main = function() {
  ...
  cat(paste(gsub("--file=", "", cmd_line[4], fixed=T),
            ...
            "\n\n"))
  option_list <- list(
    make_option(c("--filt_clustered_QCed_reclustered"), type="character", default=NULL, 
                metavar="type", 
