...
merge_and_filter_doublets = function(option_list)
{
  ...
  if (length(adatas) > 1) {
    message("Merging multiple samples...")
    merged <- merge(
      x = adatas[[1]], 
      y = adatas[-1], 
      add.cell.ids = sample_array,
      project = "Merged_scRNAseq"
    )
  } else {
    message("Only one sample detected. Skipping merge step.")
    merged <- adatas[[1]]
    # Manually add the cell ID prefix to keep naming consistent with multi-sample runs
    merged <- RenameCells(merged, add.cell.id = sample_array[1])
  }
